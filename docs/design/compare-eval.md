...
对话里的对比用同一 compare spec。实验定义在 `compare_specs` / `compare_variants` 上。
...
Online chat compare uses the same spec. Experiment definitions live on `compare_specs` / `compare_variants`.
